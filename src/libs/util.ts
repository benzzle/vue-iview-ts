 ...
 * @returns {Array}
 */
export const getMenuByRouter = (list:[], access:any) => {
...
export const showTitle = (item:any, vm:any) => {
  ...
  if (vm.$config.useI18n) {
    if (title.includes('{{') && title.includes('}}') && vm.$config.useI18n) title = title.replace(/({{[\s\S]+?}})/, (m:any, str:any) => str.replace(/{{([\s\S]*)}}/, (m:any, _:any) => vm.$t(_.trim())))
...
export const findNodeUpper:any = (ele:any, tag:any) => {
  if (ele.parentNode) {
...
export const findNodeDownward:any = (ele:any, tag:any) => {
